<!DOCTYPE html>
<html>
<head>
     <title>Traveller's Point</title>
  <link rel="icon" type="image/jpg" href="logo.jpg">
<meta name="viewport" content="width=device-width, initial-scale=1">
 <link rel="stylesheet" href="font-awesome-4.7.0/css/font-awesome.min.css">
 <link rel="stylesheet" type="text/css" href="agency_nav_style.css">
<link rel="stylesheet" type="text/css" href="drone_style.css">
    
    </head>
<body>

    <script>
function myFunction() {
    var x = document.getElementById("myTopnav");
    if (x.className === "topnav") {
        x.className += " responsive";
    } else {
        x.className = "topnav";
    }
}
</script>
    
    
    
    
    
     <div align="center">
    <h1 class="logo">Traveller's Point</h1>
    
  <strong class="logo2" >We All Travel For Life !</strong> 
    </div>
    <br>
    
<div class="topnav" id="myTopnav">
  <a href="#" ><i class="fa fa-home"></i> Home</a>
    <a href="buy_drone.php" class="active"><i class="fa fa-shopping-cart" ></i> Buy Drone</a>
  <a href="buy_tracker.php"><i class="fa fa-shopping-cart"></i> Buy Tracker</a>
  <a href="#"><i class="fa fa-upload"></i> Upload Pics</a>
  <a href="#"><i class="fa fa-edit"></i> Update Profile</a>
  <a href="javascript:void(0);" class="icon" onclick="myFunction()">
    <i class="fa fa-bars"></i>
  </a>
</div>

     <div class="column">
   <img src="quad_x.jpg" alt="Quadcopter X mode" class="drone_img">
  </div>
  <div class="column">
    <h2>Quadcopter X Mode</h2>
      <h3>Description : </h3>
    <p>Lorem ipsum dolor sit amet, consectetur adipiscing elit. Maecenas sit amet pretium urna. Vivamus venenatis velit nec neque ultricies, eget elementum magna tristique. Quisque vehicula, risus eget aliquam placerat, purus leo tincidunt eros, eget luctus quam orci in velit. Praesent scelerisque tortor sed accumsan convallis.</p>
      
  </div>
    
  <button class="button button3" type=""> BUY </button>
       
    </body>
</html>
